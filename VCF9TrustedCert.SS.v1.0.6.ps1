<#
.SYNOPSIS
VCF 9 Trusted Certificate Import UI — Minimal changes v1.0.6

.DESCRIPTION
- Keeps original 1.0.5 UI (layout unchanged; adds Posh-SSH label and button).
- Adds SSH-based import for VCF-Operations (Aria Ops for VCF).
- Enforces FQDN for all targets (blocks IPs).
- Preserves API/SDK flows for vCenter/NSX/SDDC/Installer.

.NOTES
Author:     Michael Molle
Version:    1.0.6 (Minimal UI changes + SCP/SFTP/Inline fallback + $Host fix + literal remote script)
PowerShell: 7+
#>
...
# -------------------- Self-sign & STA relaunch (kept from 1.0.5) --------------------
...
if (-not $NoRelaunch) { if ([Threading.Thread]::CurrentThread.ApartmentState -ne 'STA') { & $pwsh -NoProfile -ExecutionPolicy Bypass -STA -File "$PSCommandPath" -NoRelaunch -SignedOk; exit $LASTEXITCODE } }
  ...
  try {
    if ($script:txtLog -and $script:window) {
      if ($script:LogWarmupSync -gt 0) {
        $script:window.Dispatcher.Invoke([Action]{ try { $script:txtLog.AppendText("$line`r`n"); $script:txtLog.ScrollToEnd() } catch {} },[System.Windows.Threading.DispatcherPriority]::Render)
        $script:LogWarmupSync--
      } else {
        $null = $script:window.Dispatcher.BeginInvoke([Action]{ try { $script:txtLog.AppendText("$line`r`n"); $script:txtLog.ScrollToEnd() } catch {} })
      }
    } else { $script:logQueue.Enqueue("$line`r`n") }
  } catch {}
  Write-Host $line
}
...
function Get-CertObjectsFromFile {
  ...
  if ($ext -in '.p7b','.p7c') { $col = New-Object System.Security.Cryptography.X509Certificates.X509Certificate2Collection; $col.Import($Path); return ,@($col) }
  if ($ext -in '.cer','.crt') { $x = New-Object System.Security.Cryptography.X509Certificates.X509Certificate2($Path); return ,@($x) }
...
function Convert-CertToPem { param([System.Security.Cryptography.X509Certificates.X509Certificate2]$Cert)
...
function Test-IsCA([System.Security.Cryptography.X509Certificates.X509Certificate2]$x) {
  $isCA = $false
  foreach ($ext in $x.Extensions) { if ($ext.Oid.Value -eq '2.5.29.19') { $bc = New-Object System.Security.Cryptography.X509Certificates.X509BasicConstraintsExtension $ext, $false; if ($bc.CertificateAuthority) { $isCA = $true } break } }
  if (-not $isCA -and ($x.Subject -eq $x.Issuer)) { $isCA = $true }
  if (-not $isCA) { foreach ($ext in $x.Extensions) { if ($ext.Oid.Value -eq '2.5.29.15') { try { $ku = New-Object System.Security.Cryptography.X509Certificates.X509KeyUsageExtension $ext, $false; if (($ku.KeyUsages -band [System.Security.Cryptography.X509Certificates.X509KeyUsageFlags]::KeyCertSign) -ne 0) { $isCA = $true } } catch {} break } } }
  return $isCA
}
function Normalize-DN([string]$dn) { if ([string]::IsNullOrWhiteSpace($dn)) { return '' }; $clean = ($dn -replace '\s+',' ' -replace '\s*,\s*',',').Trim().ToLowerInvariant(); $parts = $clean.Split(',') | Where-Object { $_ -ne '' } | Sort-Object; return ($parts -join ',') }
function Get-ExpectedCAsThumbprintsFromObjects([System.Security.Cryptography.X509Certificates.X509Certificate2[]]$Certs) { $tps = New-Object System.Collections.Generic.HashSet[string]; foreach ($x in $Certs) { if (Test-IsCA $x) { [void]$tps.Add($x.Thumbprint.ToUpperInvariant()) } } return $tps }
function Get-ExpectedCNsFromObjects([System.Security.Cryptography.X509Certificates.X509Certificate2[]]$Certs) { $cns = New-Object System.Collections.Generic.HashSet[string]; foreach ($x in $Certs) { if ($x.Subject -match 'CN\s*=\s*([^,]+)') { [void]$cns.Add($Matches[1].Trim()) } } return $cns }

# -------------------- Modules: PowerCLI / VCF.PowerCLI / Posh-SSH --------------------
...
function Import-VCFPowerCLI { try { Import-Module VCF.PowerCLI -ErrorAction SilentlyContinue | Out-Null; Import-Module VMware.Sdk.Vcf.SddcManager -ErrorAction SilentlyContinue | Out-Null; Import-Module VMware.Sdk.Nsx.Policy -ErrorAction SilentlyContinue | Out-Null; Set-PowerCLIConfiguration -Scope User -InvalidCertificateAction Ignore -Confirm:$false -ErrorAction SilentlyContinue | Out-Null; return $true } catch { Write-Log "VCF.PowerCLI import failed: $($_.Exception.Message)" 'WARN'; return $false } }
...
function Ensure-Module {
  ...
  $ok = $false
  switch ($Name) {
    'VMware.PowerCLI' { $ok = Has-PowerCLI; if ($ok) { $ok = Import-PowerCLIQuiet } }
    'VCF.PowerCLI'    { $ok = Has-VCFPowerCLI; if ($ok) { $ok = Import-VCFPowerCLI } }
    'Posh-SSH'        { $ok = Has-PoshSSH;    if ($ok) { $ok = Import-PoshSSH } }
  ...
  try {
    ...
    switch ($Name) { 'VMware.PowerCLI' { $null = Import-PowerCLIQuiet }; 'VCF.PowerCLI' { $null = Import-VCFPowerCLI }; 'Posh-SSH' { $null = Import-PoshSSH } }
    Write-Log "$Name installed/updated and imported."
    return $true
  } catch { Write-Log "$Name install failed: $($_.Exception.Message)" 'ERROR'; return $false }
  finally { $ProgressPreference=$old }
}

# -------------------- Posh-SSH compatibility wrappers --------------------
function New-SSHSessionCompat {
  param(
    [Parameter(Mandatory)][string]$ComputerName,
    [Parameter(Mandatory)][pscredential]$Credential,
    [int]$Port = 22,
    [int]$ConnectionTimeout = 30,
    [int]$OperationTimeout  = 120,
    [switch]$AcceptKey
  )
  $splat = @{
  ...
  if ($AcceptKey) { $splat.AcceptKey = $true }
  $cmd = Get-Command -Name New-SSHSession -ErrorAction SilentlyContinue
  if ($cmd -and $cmd.Parameters.ContainsKey('OperationTimeout')) { $splat.OperationTimeout = $OperationTimeout }
  return New-SSHSession @splat
}

# Upload via SCP if available; fall back to SFTP; else inline via plain-text heredoc (no base64)
function Copy-RemoteFileCompat {
  param(
  ...
  # 1) Try SCP first
  $scpCmd = Get-Command -Name Set-SCPFile -ErrorAction SilentlyContinue
  if ($scpCmd) {
    Set-SCPFile -SSHSession $SSHSession -LocalFile $Local -RemotePath $Remote -ErrorAction Stop
    return
  }
  # 2) Fall back to SFTP
  $newSftp = Get-Command -Name New-SFTPSession -ErrorAction SilentlyContinue
  $putSftp = Get-Command -Name Set-SFTPFile -ErrorAction SilentlyContinue
  if ($newSftp -and $putSftp) {
    $sftp = $null
    try {
      $sftp = New-SFTPSession -ComputerName $RemoteHost -Port $Port -Credential $Credential -AcceptKey -ErrorAction Stop
      Set-SFTPFile -SFTPSession $sftp -LocalFile $Local -RemotePath $Remote -Confirm:$false -ErrorAction Stop
      return
    } finally {
      if ($sftp) { Remove-SFTPSession -SFTPSession $sftp | Out-Null }
    }
  }
  # 3) Final fallback: inline plain-text upload (PEM is ASCII; no base64 needed)
  $payload = Get-Content -Path $Local -Raw -Encoding ASCII
  $token   = "SSLINSPECT_{0}" -f ([Guid]::NewGuid().ToString('N'))
  $upload  = "cat > `"$Remote`" <<'$token'" + "`n" + $payload + "`n$token`n"
  Invoke-SSHCommand -SSHSession $SSHSession -Command $upload -TimeOut 300 -ErrorAction Stop | Out-Null
}

# -------------------- API helpers (Installer/SDDC/NSX) --------------------
function Get-VcfInstallerToken { param([string]$Fqdn,[string]$User,[string]$Pass); $body = @{ username=$User; password=$Pass } | ConvertTo-Json; Invoke-RestMethod -Uri ("https://{0}/v1/tokens" -f $Fqdn) -Method Post -Body $body -ContentType 'application/json' -SkipCertificateCheck -ErrorAction Stop }
function Add-VcfInstallerTrustedCert {
  ...
  foreach ($x in $Certs) {
    ...
    try { Invoke-RestMethod -Uri ("https://{0}/v1/sddc-manager/trusted-certificates" -f $Fqdn) -Method Post -Headers $hdrs -Body $body -SkipCertificateCheck -ErrorAction Stop | Out-Null; $added++ }
    catch { $status=$null; try { $status=[int]$_.Exception.Response.StatusCode } catch {}; if ($status -eq 409 -or $_.Exception.Message -match '\b409\b') { $skipped++; Write-Log "[$Fqdn] Installer API: certificate already present (409)." } else { throw } }
  }
...
function Add-SddcTrustedCertificateSDK {
  ...
  if (-not (Ensure-Module -Name 'VCF.PowerCLI')) { throw "VCF.PowerCLI not available." }
  $sd = $null
  try {
...
function Invoke-NSXCaImport {
  ...
  catch { $status = $null; try { $status = [int]$_.Exception.Response.StatusCode } catch {}; if ($status -eq 409 -or $_.Exception.Message -match '\b409\b') { Write-Log "[$Fqdn] NSX API: certificate(s) already present (409)."; [pscustomobject]@{Added=0;Skipped=1;Message='CA bundle already present (idempotent).'} } else { throw } }
}

# -------------------- VCF-Operations over SSH (Posh-SSH) --------------------
function Invoke-VCFOpsTrustedImport {
  param(
    [Parameter(Mandatory)][string]$HostFqdn,
    [Parameter(Mandatory)][string]$Username,
    [Parameter(Mandatory)][string]$Password,
    [int]$Port = 22,
    [Parameter(Mandatory)][System.Security.Cryptography.X509Certificates.X509Certificate2[]]$Certs
  )
  if (-not (Ensure-Module -Name 'Posh-SSH')) { throw "Posh-SSH module not available." }

  # Build combined PEM chain
  $pemText = ($Certs | ForEach-Object { Convert-CertToPem $_ }) -join ""
  $localPem = Join-Path $script:RunDir "vcfops-chain.pem"
  Set-Content -Path $localPem -Value $pemText -Encoding ASCII -Force

  # SSH + Upload (SCP/SFTP/Inline)
  $sec = ConvertTo-SecureString $Password -AsPlainText -Force
  $cred = New-Object System.Management.Automation.PSCredential($Username,$sec)
  $sess = $null
  try {
    $sess = New-SSHSessionCompat -ComputerName $HostFqdn `
            -Port $Port -Credential $cred -AcceptKey `
            -ConnectionTimeout 30 -OperationTimeout 120
  } catch { throw "SSH connect failed: $($_.Exception.Message)" }

  try {
    Copy-RemoteFileCompat -RemoteHost $HostFqdn -Port $Port -Credential $cred `
      -SSHSession $sess -Local $localPem -Remote "/tmp/sslinspect-chain.pem"
  } catch {
    if ($sess) { Remove-SSHSession -SSHSession $sess | Out-Null }
    throw "SCP/SFTP/Inline upload failed: $($_.Exception.Message)"
  }

  # Remote import script — LITERAL (no PowerShell interpolation)
...
CACERTS='/usr/lib/vmware-vcops/user/conf/ssl/cacerts'
BACKUP="${CACERTS}.$(date +%Y%m%d%H%M%S).bak"

# Helper: run as root or sudo with password (decode only if sudo is required)
if [ "$(id -u)" -eq 0 ]; then
  RUN() { "$@"; }
else
  if command -v base64 >/dev/null 2>&1; then
    SUDOPW=$(echo -n "$PW_B64" | base64 -d 2>/dev/null || true)
  elif command -v openssl >/dev/null 2>&1; then
    SUDOPW=$(echo -n "$PW_B64" | openssl base64 -d -A 2>/dev/null || true)
  else
    SUDOPW=""
  fi
  RUN() { echo "$SUDOPW" | sudo -S "$@"; }
fi

command -v openssl >/dev/null 2>&1 || { echo "openssl not found"; exit 2; }
command -v keytool  >/dev/null 2>&1 || { echo "keytool not found"; exit 3; }

# Normalize Windows line endings if present (best-effort)
sed -i 's/\r$//' "$PEM" 2>/dev/null || true

# Backup truststore (best-effort)
RUN cp -p "$CACERTS" "$BACKUP" 2>/dev/null || true

# Split PEM into individual files
awk 'BEGIN{c=0}/-----BEGIN CERTIFICATE-----/{c++;f=sprintf("/tmp/sslcert-%02d.pem",c)}{print > f}/-----END CERTIFICATE-----/{close(f)}' "$PEM"

# Import each cert with fingerprint-based alias to dedupe
for f in /tmp/sslcert-*.pem; do
  [ -f "$f" ] || continue
  alias=$(openssl x509 -in "$f" -noout -fingerprint -sha1 | sed 's/.*=//' | tr -d ':' | tr '[:upper:]' '[:lower:]')
  if RUN keytool -list -keystore "$CACERTS" -storepass changeit | grep -qi "sslinspect_$alias"; then
    echo "alias sslinspect_$alias already present, skipping"
    continue
  fi
  RUN keytool -importcert -trustcacerts -noprompt -alias "sslinspect_$alias" -file "$f" -keystore "$CACERTS" -storepass changeit
done

# Restart service
if command -v systemctl >/dev/null 2>&1; then
  RUN systemctl restart vmware-vcops
else
  RUN service vmware-vcops restart
fi

echo "VCF-Operations truststore updated."
'@

  # Inject the password token safely
  $remoteScript = $remoteScript.Replace('__PW64_TOKEN__', $pwB64)

  try {
    $res = Invoke-SSHCommand -SSHSession $sess -Command $remoteScript -TimeOut 900 -ErrorAction Stop
    $msg = ($res.Output | Where-Object { $_ -and $_.Trim().Length -gt 0 }) -join '; '
    if (-not $msg) { $msg = "VCF-Operations truststore updated (no output)" }
    return [pscustomobject]@{ Added=1; Skipped=0; Message=$msg }
  } catch { throw "Remote import failed: $($_.Exception.Message)" }
  finally { if ($sess) { Remove-SSHSession -SSHSession $sess | Out-Null } }
}
...
function Get-TypeDefaults { param([string]$Type)
  switch ($Type) {
    ...
    'VCF-Operations' { @{ Port = 22;  Username = 'root' } }  # default = root
...
# -------------------- FQDN enforcement --------------------
$script:EnforceFqdn = $true
function Test-IsIPAddress([string]$s) { if ([string]::IsNullOrWhiteSpace($s)) { return $false }; $ipObj=$null; [System.Net.IPAddress]::TryParse($s,[ref]$ipObj) }
function Test-IsFqdn([string]$s) {
  if ([string]::IsNullOrWhiteSpace($s)) { return $false }
  if (Test-IsIPAddress $s) { return $false }
  return ($s -match '^(?=.{1,253}$)(?:[A-Za-z0-9\-]{0,61}[A-Za-z0-9]?\.)+[A-Za-z]{2,}$')
}
function Validate-FqdnOrFail {
  param([System.Collections.ObjectModel.ObservableCollection[psobject]]$Targets)
  $bad = @()
  foreach ($t in $Targets) {
    $hostName = [string]$t.Host   # avoid $Host automatic var
    if ($script:EnforceFqdn -and (-not (Test-IsFqdn $hostName))) { $bad += $hostName }
  }
  if ($bad.Count -gt 0) {
    $list = ($bad | Sort-Object -Unique) -join ', '
    throw "FQDN enforcement is ON. The following Host entries are not valid FQDNs (likely IPs): $list"
  }
}

# -------------------- WPF UI (same layout + minimal additions for Posh-SSH) --------------------
   ...
   <Grid>
    ...
    <Grid Grid.Column="0">
     ...
     <StackPanel Grid.Column="1">
      ...
      <!-- Minimal addition: Posh-SSH prereq label -->
    ...
    <StackPanel Grid.Column="3" Orientation="Vertical" VerticalAlignment="Center">
     ...
     <!-- Minimal addition: Install Posh-SSH button -->
     <Button x:Name="btnInstallPosh" Content="Install Posh-SSH" MinWidth="170"/>
    </StackPanel>
   </Grid>
  </GroupBox>
...
"@
try { $script:window = [Windows.Markup.XamlReader]::Parse($xaml) } catch { [System.Windows.MessageBox]::Show("XAML parse failed:`r`n$($_.Exception.Message)","XAML Error",[System.Windows.MessageBoxButton]::OK,[System.Windows.MessageBoxImage]::Error) | Out-Null; throw }
$script:window.WindowState='Maximized'

# -------------------- Bind controls (includes Posh-SSH controls) --------------------
...
$script:lblPoshSSH = $script:window.FindName('lblPoshSSH')
$script:btnRecheck = $script:window.FindName('btnRecheck')
$script:btnInstallPCLI = $script:window.FindName('btnInstallPCLI')
$script:btnInstallVCF = $script:window.FindName('btnInstallVCF')
$script:btnInstallPosh = $script:window.FindName('btnInstallPosh')
...
$script:uiTimer.add_Tick({
  try {
    ...
    while ($true) { if (-not $script:logQueue.TryDequeue([ref]$line)) { break }; [void]$sb.Append($line) }
    if ($sb.Length -gt 0 -and $script:txtLog) { $script:txtLog.AppendText($sb.ToString()); $script:txtLog.ScrollToEnd() }
  } catch {}
})
try { $script:uiTimer.Start() } catch {}
$script:window.Add_ContentRendered({
  try { if (-not $script:RunDir) { $null = New-RunDir -Base $script:ReportsBase }; if ($script:txtReports) { $script:txtReports.Text = $script:ReportsBase }; Write-Log "==== SSLInspect UI started ===="; Write-Log "Run folder: $script:RunDir" } catch {}
  try { $script:uiTimer.Start() } catch {}
...
function Prereq-Check {
  ...
  Set-StatusText -Label $script:lblPS -Text ("PowerShell {0}" -f $PSVersionTable.PSVersion) -State $(if($isPS7){'OK'}else{'FAIL'}); $ok = $ok -and $isPS7
  ...
  $hasPosh = Has-PoshSSH; if ($hasPosh) { Import-PoshSSH | Out-Null }
  if ($script:lblPoshSSH) {
    Set-StatusText -Label $script:lblPoshSSH -Text $(if($hasPosh){"Posh-SSH: Found"}else{"Posh-SSH: Not found"}) -State $(if($hasPosh){'OK'}else{'WARN'})
  }
...
if ($script:btnInstallPCLI) { $script:btnInstallPCLI.Add_Click({ Ensure-Module -Name 'VMware.PowerCLI' | Out-Null; Prereq-Check | Out-Null }) }
if ($script:btnInstallVCF)  { $script:btnInstallVCF.Add_Click({ Ensure-Module -Name 'VCF.PowerCLI'   | Out-Null; Prereq-Check | Out-Null }) }
if ($script:btnInstallPosh) { $script:btnInstallPosh.Add_Click({ Ensure-Module -Name 'Posh-SSH'       | Out-Null; Prereq-Check | Out-Null }) }
if ($script:btnBrowseReports) { 
  $script:btnBrowseReports.Add_Click({
    try {
      ...
      if ($dlg.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { $script:ReportsBase = $dlg.SelectedPath; if ($script:txtReports) { $script:txtReports.Text = $script:ReportsBase } }
    } catch {}
  })
}
...
function Capture-Passwords {
  try {
    for ($i=0; $i -lt $script:gridTargets.Items.Count; $i++) {
      ...
      if ($pb) { $row.Password = $pb.Password } else { Write-Log "Row $($i): PasswordBox not realized — continuing." 'WARN' }
    }
...
if ($script:btnBrowseChain) { $script:btnBrowseChain.Add_Click({ $dlg = New-Object System.Windows.Forms.OpenFileDialog; $dlg.Filter = "Cert files (*.cer;*.crt;*.pem;*.p7b)|*.cer;*.crt;*.pem;*.p7b|All files (*.*)|*.*"; if ($dlg.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { $script:txtChain.Text = $dlg.FileName } }) }
...
# -------------------- Test (FQDN enforced; VCF-Operations uses SSH) --------------------
function Test-Target {
  param([string]$Type,[string]$TargetHost,[string]$User,[string]$Pass,[int]$Port)
  $Type = Normalize-TargetType $Type
  if ($script:EnforceFqdn -and -not (Test-IsFqdn $TargetHost)) { return [pscustomobject]@{Host=$TargetHost;Type=$Type;Step='Test';Status='Fail';Message='Host must be a FQDN (IP not allowed)'} }
  switch ($Type) {
    'vCenter' {
      try {
    ...
    'VCF-Operations' {
      try {
        if (-not (Ensure-Module -Name 'Posh-SSH')) { throw "Posh-SSH not available." }
        $sec = ConvertTo-SecureString $Pass -AsPlainText -Force
        $cred = New-Object System.Management.Automation.PSCredential($User,$sec)
        $sess = $null
        try {
          $sess = New-SSHSessionCompat -ComputerName $TargetHost `
                  -Port ($Port -gt 0 ? $Port : 22) `
                  -Credential $cred -AcceptKey `
                  -ConnectionTimeout 30 -OperationTimeout 60
          $rc = Invoke-SSHCommand -SSHSession $sess -Command "echo ok" -TimeOut 10 -ErrorAction Stop
          if ($rc.Output -match 'ok') { [pscustomobject]@{Host=$TargetHost;Type=$Type;Step='SSH';Status='Pass';Message='SSH reachable'} } else { [pscustomobject]@{Host=$TargetHost;Type=$Type;Step='SSH';Status='Fail';Message='SSH responded unexpectedly'} }
        } finally { if ($sess) { Remove-SSHSession -SSHSession $sess | Out-Null } }
      } catch { [pscustomobject]@{Host=$TargetHost;Type=$Type;Step='SSH';Status='Fail';Message=$_.Exception.Message} }
    }
...
if ($script:btnTest) {
  $script:btnTest.Add_Click({
    try {
      ...
      try { Validate-FqdnOrFail -Targets $script:TargetsOC } catch { $script:Rows.Add([pscustomobject]@{Host='(all)';Type='(validation)';Step='Test';Status='Fail';Message=$_.Exception.Message}) | Out-Null; Write-Log $_.Exception.Message 'ERROR'; Write-Log "==== Test connection finished ===="; return }
...
# -------------------- Run (install) --------------------
function Write-ChainDebug { param([System.Security.Cryptography.X509Certificates.X509Certificate2[]]$Certs,[string]$OutPath)
  $rows = foreach ($x in $Certs) { [pscustomobject]@{ Subject=$x.Subject; Issuer=$x.Issuer; SelfSigned=($x.Subject -eq $x.Issuer); IsCA=(Test-IsCA $x); Thumbprint=$x.Thumbprint; NotBefore=$x.NotBefore; NotAfter=$x.NotAfter } }
  try { $rows | Format-Table | Out-String | Set-Content -Path $OutPath -Encoding UTF8; Write-Log "Chain debug written: $OutPath" } catch { Write-Log "Chain debug write failed: $($_.Exception.Message)" 'WARN' }
}

if ($script:btnRun) {
  $script:btnRun.Add_Click({
    try {
      if (-not (Prereq-Check)) { [System.Windows.MessageBox]::Show("Prerequisites not met. Install required modules first.","SSLInspect",'OK','Warning') | Out-Null; return }
      Capture-Passwords
      if (-not (Test-Path $script:txtChain.Text)) { throw "Chain file not found." }

      Validate-FqdnOrFail -Targets $script:TargetsOC
      ...
      try {
        ...
        if ($expectedCATPs.Count -eq 0) { $lines.Add("NO CA certificates detected in uploaded chain") | Out-Null } else { foreach ($tp in $expectedCATPs) { $lines.Add($tp) | Out-Null } }
      ...
      foreach ($t in $script:TargetsOC) {
        Apply-TypeDefaults -Row $t
        $targetHost = $t.Host; $targetType = (Normalize-TargetType $t.Type); $user = $t.Username; $pass = $t.Password; $port = [int]$t.Port
          ...
          switch ($targetType) {
            'vCenter' {
              ...
              try {
                ...
                try { Add-VITrustedCertificate -Server $vi -VCenterOnly -PemCertificateOrChain $pemText -Confirm:$false -ErrorAction Stop | Out-Null } finally { $ConfirmPreference=$old }
            ...
            'VCF-Operations' {
              Write-Log "[$targetHost] Importing trusted CA chain into VCF-Operations (SSH)…"
              $res = Invoke-VCFOpsTrustedImport -HostFqdn $targetHost -Username $user -Password $pass -Port ($port -gt 0 ? $port : 22) -Certs $certObjs
              $installStatus = if ($res.Added -gt 0){'Pass'} else {'Info'}
              $script:Rows.Add(([pscustomobject]@{Host=$targetHost;Type=$targetType;Step='Install';Status=$installStatus;Message=$res.Message})) | Out-Null
            }
            default {
      ...
      $normalized = foreach ($r in $script:Rows) { [pscustomobject]@{ Host=$r.Host; Type=$r.Type; Step=$r.Step; Status=$r.Status; Message=$r.Message } }
